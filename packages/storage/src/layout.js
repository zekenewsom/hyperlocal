...
export function storageRoot(cfg = getCfg()) {
    return resolveRepoPath(cfg.storage_root);
}
export function parquetRoot(cfg = getCfg()) {
    return path.join(storageRoot(cfg), 'parquet');
}
export function dbPath(cfg = getCfg()) {
    return path.join(storageRoot(cfg), 'hyperliquid.duckdb');
}
export function dirFor(coin, interval, date, cfg = getCfg()) {
